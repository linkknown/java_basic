...
//		lst.add(new String("helloworld"));
	...
	private <T> T generateRandomInGenericArray (T[] objectArr) {
		// 0 ~ objectArr.length - 1
		return objectArr[new Random().nextInt(objectArr.length)];			
	}
...
//		Fibonacci fibonacci = new Fibonacci();
		// 把 Fibnaccio 数列当成普通数列来对待，至于底层实现，毫不关心
		Generator<Integer> fibonacci = new Fibonacci();			
		for (int i = 0; i < 100; i++) {
...
/************************************* 泛型擦除 **************************************************/
	 ...
	 */
	@Test
	public void testGenericErase2 () throws Exception {
	 ...
	 */
	@Test
	public void testGenericTypeInfer() {
		...
		Number random4 = getRandom(1, 1.2); // 这两个参数一个是Integer，一个风格是 double，所以取同一父类的最小级，为Number
		Object random5 = getRandom(1, "hello"); // 这两个参数一个是Integer，一个风格是double，所以取同一父类的最小级，为Object
	}
